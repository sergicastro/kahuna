...
from org.jclouds.abiquo.predicates.infrastructure import RemoteServicePredicates
...
class Manager:
    ...
    def get_datacenter_by_rsip(self, datacenters, rsip, context):
        """ Returns the datacenter with Node Collector
        and Virtual System Monitor in the given ip.
        If it is not found it will be created. """

        selected_dc = None
        for dc in datacenters:
            nc = dc.findRemoteService(RemoteServicePredicates.type(
                RemoteServiceType.NODE_COLLECTOR))
            if nc and nc.getUri().find(rsip) >= 0:
                self.__logger.debug(
                        "Node Collector [%s] found in datacenter '%s'."
                        % (rsip, dc.getName()))
                vsm = dc.findRemoteService(RemoteServicePredicates.type(
                    RemoteServiceType.VIRTUAL_SYSTEM_MONITOR))
                if vsm and vsm.getUri().find(rsip) >= 0:
                    self.__logger.debug(
                            "Virtual System Monitor [%s]" +
                            "found in datacenter '%s'."
                            % (rsip, dc.getName()))
                    selected_dc = dc
                    break

        if not selected_dc:
            self.__logger.debug("No datacenter with NC and VSM in '%s' found."
                    % rsip)
            ...
            try:
                ...
                if ex.hasError("RS-3"):
                    print ("Ip %s to create remote services has" +
                    "been used yet, try with another one") % rsip
                    dc.delete()
                    return
                else:
                    raise ex
            ...
            if not rack:
                rack = Rack.builder(context, selected_dc) \
                        .name('Volcano').build()
                rack.save()
            self.__logger.debug("Datacenter '%s' found. "
                    % selected_dc.getName())
            return selected_dc
    ...
    def enable_disable_datastore(self, datastore, uuid, boolean_value):
        ...
        if datastore.getDatastoreUUID() == uuid:
            ...
            self.__logger.debug("%sabling datastore '%s'" %
                    ("en" if boolean_value else "dis",
                        datastore.getRootPath()))
        return datastore
