...
def pprint_datastores(machines):
    ...
    table = [["hypervisor", "address", "machine", "id", "name", "enabled",
        "size (used/total)", "rootPath", "UUID"]]
    for machine in machines:
        for ds in machine.getDatastores():
            row = [machine.getType().name(), machine.getName(),
                    machine.getIp(),
                    ds.getId(), ds.getName(), ds.isEnabled(),
                    str(ds.getUsedSize() / 1024 / 1024) + " / " +
                    str(ds.getSize() / 1024 / 1024) + " MB",
